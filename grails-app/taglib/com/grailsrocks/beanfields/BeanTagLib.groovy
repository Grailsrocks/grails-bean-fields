...
class BeanTagLib {
    ...
    static final Closure DEFAULT_HIDDEN_RENDERING = { args ->
        args.field
    }
	...
	static final Map DEFAULT_PARAMS = Collections.unmodifiableMap([
        ...
        HIDDEN_TEMPLATE: DEFAULT_HIDDEN_RENDERING
	])
    ...
    def hidden = { attrs ->
        ...
        doTag( attrs, { renderParams ->
            ...
            def hiddenField = g.hiddenField(attrs)

            // Use the current template closure if set
            out << tagInfo.HIDDEN_TEMPLATE.clone().call(field:hiddenField,
                bean: renderParams.bean,
                beanName: renderParams.beanName,
                propertyName: renderParams.propertyName)
        })
	...
	def select = { attrs ->
		...
		doTag( attrs, { renderParams ->
		    ...
		    if (domainArtefact) {
		        ...
		        propType = renderParams.bean.metaClass.getMetaProperty(renderParams.propertyName).type
	        }
    ...
    def resolveBeanAndProperty(attrs) {
		...
		if (!attrs._BEAN.bean) {
            throwTagError "No bean found in page scope for [${attrs._BEAN.beanName}] and property [${attrs._BEAN.propertyName}]"
		}
    }
